...
if [ "$?" -eq "0" ]; then
  ...
  mysql -u root --password=${MYSQL_ROOT_PASSWORD} -h db < /keystone-init/keystone.sql
else
...
sleep 5
RESULT=$(keystone --os-endpoint http://keystone:35357/v2.0 --os-token ddc06b242b0718b495c7 service-list | grep http://keystone:35357 | wc -l)
if [ $RESULT -ne 1 ]; then
